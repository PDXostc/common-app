...
function BuildItemHTML(itemPhoto,itemTitle,itemDesc){
    ...
    output += '<div class=\"article-list-item textBgColorThemeTransparent\">';
    output += '<div class=\"thumbnail\" ontouchend=\"showArticle(\''+ itemFeed +'/did='+ itemGuid +'\')\"><img src=\"'+ itemPhoto +'\" width=\"180px\" \/></div>';
    output += '<div class=\"item-text\"><span class=\"item-date fontSizeXXSmall fontColorTheme\">'+ d + m + y +'</span><br/><span ontouchend=\"showArticle(\''+ itemFeed +'/did='+ itemGuid +'\')\" class=\"item-title fontSizeSmall fontColorNormal\">'+ itemTitle +'</span><br/>';
    output += '<span class=\"item-desc fontSizeXSmall fontColorNormal\">'+ itemDesc +'</span>';
    output += '</div><div class=\"item-text-grad-fade\"></div></div>';
    output += '</li>';
    return output;
}
...
function showArticle(url) {
    ...
    */
	...
	$('item',xml).each(function(i) {
            ...
            articleBody = articleBody.replace(/(?:^|[^"'])((ftp|http|https|file):\/\/[\S]+(\b|$))/gi, "<p><img width=\"627px\" class=\"article-photo\" src=\"$1\" />");
            itemId = $(this).find("id").text();
        });

        myHTMLOutput = '<li class=\"articleItem\"><div class=\"article-body fontSizeLarge fontColorNormal\">';
        myHTMLOutput += '<span class=\"article-head fontSizeLarger fontColorNormal\">'+ articleHead +'</span>'+ articleBody +'</div></li>';
