 ...
 * * {{#crossLink "Settings.Themes"}}{{/crossLink}} component
 * * {{#crossLink "Settings.Wifi"}}{{/crossLink}} component
 * * {{#crossLink "Settings.Bluetooth"}}{{/crossLink}} component
 * 
 * @class Settings
 * @module Settings
 * @namespace Settings
	...
	Settings.prototype.renderSettingsView = function(successCallback) {
		...
		loadTemplate(self.SETTINGS_TEMPLATES_PATH, templateName, function() {
			if (!$("#settingsList").length) {
				...
				//ko.applyBindings(window.Settings, self.domElement.get(0));
			}
			if (!!successCallback) {
				successCallback();
			}
