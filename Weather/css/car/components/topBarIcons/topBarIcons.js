...
function launchApplication(id) {
	...
	if ( !! app) {
		if( app != tizen.application.getCurrentApplication() ) 
		 {
			// This is not completely understood, but calling launch twice is necessary in order to reliably
			// transition from an app. to HomeScreen and back to the same app. And this also allows reliable navigation
			// from one app. to another using the icons from HomeScreen or any other app.
			// When the app. is already running, the launch call just brings it to the front. Calling it twice
			// brings it to the front in a state wher it responds to touches.
