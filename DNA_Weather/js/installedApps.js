			...
			if (appList[i].id === appData.id) {
				if (appData.id === "http://com.intel.tizen/intelPocSettings") {
					if (typeof Settings === 'undefined') {
						...
						// Implement the "tab fix" which requires that the app. be launched twice when it's icon (or grid button)
						// is activated, when the app. is already running. This makes the app come to the foreground.
						// so that the "tab fix" workaround of launching twice can be performed.
						var app = getAppByID(appData.id);
